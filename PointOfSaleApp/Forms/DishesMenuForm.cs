    ...
    public partial class DishesMenuForm : Form
    {
        ...
        private void categoryButtonClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataRow row in table.Rows)
                {
                    Classes.CategoryClass.categoryId = int.Parse(row["id"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void loadDishDataGridView()
        {
            ...
            string query = "select Dish.* from [Dish] join Dish_Category on Dish.id = Dish_Category.dish_id join Category on Dish_Category.category_id = Category.id where category_id = " + Classes.CategoryClass.categoryId;
            SqlCommand command = new SqlCommand(query, conn);
            conn.Open();
            SqlDataAdapter adapter = new SqlDataAdapter(command);
            DataTable table = new DataTable();
        ...
        {
            // Classes.SelectedDishClass.dishId 
        }
